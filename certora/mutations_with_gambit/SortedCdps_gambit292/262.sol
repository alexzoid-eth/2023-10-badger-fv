...
contract SortedCdps is ISortedCdps {
    ...
    function _ascendList(uint256 _NICR, bytes32 _startId) internal view returns (bytes32, bytes32) {
        ...
        /// IfStatementMutation(`data.tail == _startId && _NICR <= cdpManager.getCachedNominalICR(_startId)` |==> `false`) of: `if (data.tail == _startId && _NICR <= cdpManager.getCachedNominalICR(_startId)) {`
        if (false) {
