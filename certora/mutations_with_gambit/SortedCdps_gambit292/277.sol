...
contract SortedCdps is ISortedCdps {
    ...
    function _findInsertPosition(
        ...
        if (nextId != dummyId) {
            /// IfStatementMutation(`!contains(nextId) || _NICR < cdpManager.getCachedNominalICR(nextId)` |==> `true`) of: `if (!contains(nextId) || _NICR < cdpManager.getCachedNominalICR(nextId)) {`
            if (true) {
