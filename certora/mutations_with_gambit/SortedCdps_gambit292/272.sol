...
contract SortedCdps is ISortedCdps {
    ...
    function _findInsertPosition(
        ...
        if (prevId != dummyId) {
            /// SwapArgumentsOperatorMutation(`_NICR > cdpManager.getCachedNominalICR(prevId)` |==> `cdpManager.getCachedNominalICR(prevId) > _NICR`) of: `if (!contains(prevId) || _NICR > cdpManager.getCachedNominalICR(prevId)) {`
            if (!contains(prevId) || cdpManager.getCachedNominalICR(prevId) > _NICR) {
