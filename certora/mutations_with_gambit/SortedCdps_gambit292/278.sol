...
contract SortedCdps is ISortedCdps {
    ...
    function _findInsertPosition(
        ...
        if (nextId != dummyId) {
            /// IfStatementMutation(`!contains(nextId) || _NICR < cdpManager.getCachedNominalICR(nextId)` |==> `false`) of: `if (!contains(nextId) || _NICR < cdpManager.getCachedNominalICR(nextId)) {`
            if (false) {
