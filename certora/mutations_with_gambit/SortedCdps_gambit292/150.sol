...
contract SortedCdps is ISortedCdps {
    ...
    function batchRemove(bytes32[] memory _ids) external override {
        ...
        /// RequireMutation(`_len > 1` |==> `false`) of: `require(_len > 1, "SortedCdps: batchRemove() only apply to multiple cdpIds!");`
        require(false, "SortedCdps: batchRemove() only apply to multiple cdpIds!");
