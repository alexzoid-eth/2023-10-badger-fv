...
contract EBTCToken is IEBTCToken, AuthNoOwner, PermitNonce {
    ...
    function permit(
        ...
        /// SwapArgumentsOperatorMutation(`deadline >= block.timestamp` |==> `block.timestamp >= deadline`) of: `require(deadline >= block.timestamp, "EBTC: expired deadline");`
        require(block.timestamp >= deadline, "EBTC: expired deadline");
