...
contract EBTCToken is IEBTCToken, AuthNoOwner, PermitNonce {
    ...
    function _burn(address account, uint256 amount) internal {
        /// DeleteExpressionMutation(`require(account != address(0), "EBTCToken: burn from zero account!")` |==> `assert(true)`) of: `require(account != address(0), "EBTCToken: burn from zero account!");`
        assert(true);
