...
contract CollSurplusPool is ICollSurplusPool, ReentrancyGuard, AuthNoOwner {
    ...
    function claimSurplusCollShares(address _account) external override {
        ...
        /// DeleteExpressionMutation(`require(claimableColl > 0, "CollSurplusPool: No collateral available to claim")` |==> `assert(true)`) of: `require(claimableColl > 0, "CollSurplusPool: No collateral available to claim");`
        assert(true);
