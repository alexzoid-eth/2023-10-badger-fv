...
contract CollSurplusPool is ICollSurplusPool, ReentrancyGuard, AuthNoOwner {
    ...
    function sweepToken(address token, uint256 amount) public nonReentrant requiresAuth {
        ...
        /// DeleteExpressionMutation(`require(amount <= balance, "CollSurplusPool: Attempt to sweep more than balance")` |==> `assert(true)`) of: `require(amount <= balance, "CollSurplusPool: Attempt to sweep more than balance");`
        assert(true);
