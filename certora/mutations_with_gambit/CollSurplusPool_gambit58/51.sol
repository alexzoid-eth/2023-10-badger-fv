...
contract CollSurplusPool is ICollSurplusPool, ReentrancyGuard, AuthNoOwner {
    ...
    function sweepToken(address token, uint256 amount) public nonReentrant requiresAuth {
        /// DeleteExpressionMutation(`require(token != address(collateral), "CollSurplusPool: Cannot Sweep Collateral")` |==> `assert(true)`) of: `require(token != address(collateral), "CollSurplusPool: Cannot Sweep Collateral");`
        assert(true);
