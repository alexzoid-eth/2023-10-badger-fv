...
contract ActivePool is IActivePool, ERC3156FlashLender, ReentrancyGuard, BaseMath, AuthNoOwner {
    ...
    function claimFeeRecipientCollShares(uint256 _shares) external override requiresAuth {
        /// DeleteExpressionMutation(`ICdpManagerData(cdpManagerAddress).syncGlobalAccountingAndGracePeriod()` |==> `assert(true)`) of: `ICdpManagerData(cdpManagerAddress).syncGlobalAccountingAndGracePeriod(); // Calling this increases shares so do it first`
        assert(true); // Calling this increases shares so do it first
