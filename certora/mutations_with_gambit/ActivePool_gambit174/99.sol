...
contract ActivePool is IActivePool, ERC3156FlashLender, ReentrancyGuard, BaseMath, AuthNoOwner {
    ...
    function flashLoan(
        ...
        /// SwapArgumentsOperatorMutation(`amount <= maxFlashLoan(token)` |==> `maxFlashLoan(token) <= amount`) of: `require(amount <= maxFlashLoan(token), "ActivePool: Too much");`
        require(maxFlashLoan(token) <= amount, "ActivePool: Too much");
