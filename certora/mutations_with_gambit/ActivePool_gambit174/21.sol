...
contract ActivePool is IActivePool, ERC3156FlashLender, ReentrancyGuard, BaseMath, AuthNoOwner {
    ...
    function transferSystemCollSharesAndLiquidatorReward(
        ...
        uint256 cachedSystemCollShares = systemCollShares;
        /// SwapArgumentsOperatorMutation(`cachedSystemCollShares >= _shares` |==> `_shares >= cachedSystemCollShares`) of: `require(cachedSystemCollShares >= _shares, "ActivePool: Insufficient collateral shares");`
        require(_shares >= cachedSystemCollShares, "ActivePool: Insufficient collateral shares");
