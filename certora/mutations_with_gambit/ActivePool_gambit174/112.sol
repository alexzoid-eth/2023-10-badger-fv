...
contract ActivePool is IActivePool, ERC3156FlashLender, ReentrancyGuard, BaseMath, AuthNoOwner {
    ...
    function flashLoan(
        ...
        require(
            /// SwapArgumentsOperatorMutation(`collateral.balanceOf(address(this)) >= collateral.getPooledEthByShares(systemCollShares)` |==> `collateral.getPooledEthByShares(systemCollShares) >= collateral.balanceOf(address(this))`) of: `collateral.balanceOf(address(this)) >= collateral.getPooledEthByShares(systemCollShares),`
            collateral.getPooledEthByShares(systemCollShares) >= collateral.balanceOf(address(this)),
