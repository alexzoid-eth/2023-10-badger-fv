...
contract CollSurplusPool is ICollSurplusPool, ReentrancyGuard, AuthNoOwner {
    ...
    function sweepToken(address token, uint256 amount) public nonReentrant requiresAuth {
        ...
        /// SwapArgumentsOperatorMutation(`amount <= balance` |==> `balance <= amount`) of: `require(amount <= balance, "CollSurplusPool: Attempt to sweep more than balance");`
        require(balance <= amount, "CollSurplusPool: Attempt to sweep more than balance");
