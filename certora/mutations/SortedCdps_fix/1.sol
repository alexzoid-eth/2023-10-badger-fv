...
index f731a83..235f504 100644
--- a/packages/contracts/contracts/SortedCdps.sol
+++ b/packages/contracts/contracts/SortedCdps.sol
@@ -176,6 +176,9 @@ contract SortedCdps is ISortedCdps {
         bytes32 startNodeId,
         uint maxNodes
     ) internal view returns (bytes32, bool) {
+        if(startNodeId != dummyId) {
+            require(contains(startNodeId), "SortedCdps: List does not contain the id");
+        }
...
@@ -239,6 +242,9 @@ contract SortedCdps is ISortedCdps {
         bytes32 startNodeId,
         uint maxNodes
...
+        }
         ...
         // start at the given node or from the tail of list
         bytes32 _currentCdpId = (startNodeId == dummyId ? data.tail : startNodeId);
@@ -305,7 +311,10 @@ contract SortedCdps is ISortedCdps {
         if (maxArraySize == 0) {
