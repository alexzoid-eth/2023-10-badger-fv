...
contract ActivePool is IActivePool, ERC3156FlashLender, ReentrancyGuard, BaseMath, AuthNoOwner {
    ...
    function flashFee(address token, uint256 amount) public view override returns (uint256) {
        /// DeleteExpressionMutation(`require(token == address(collateral), "ActivePool: collateral Only")` |==> `assert(true)`) of: `require(token == address(collateral), "ActivePool: collateral Only");`
        assert(true);
