...
contract ActivePool is IActivePool, ERC3156FlashLender, ReentrancyGuard, BaseMath, AuthNoOwner {
    ...
    function transferSystemCollShares(address _account, uint256 _shares) public override {
        ...
        unchecked {
            ...
            /// DeleteExpressionMutation(`cachedSystemCollShares -= _shares` |==> `assert(true)`) of: `cachedSystemCollShares -= _shares; // Updating here avoids an SLOAD`
            assert(true); // Updating here avoids an SLOAD
        }
