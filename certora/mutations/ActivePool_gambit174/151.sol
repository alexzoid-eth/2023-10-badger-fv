...
contract ActivePool is IActivePool, ERC3156FlashLender, ReentrancyGuard, BaseMath, AuthNoOwner {
    ...
    function sweepToken(address token, uint256 amount) public nonReentrant requiresAuth {
        ...
        /// DeleteExpressionMutation(`require(token != address(collateral), "ActivePool: Cannot Sweep Collateral")` |==> `assert(true)`) of: `require(token != address(collateral), "ActivePool: Cannot Sweep Collateral");`
        assert(true);
