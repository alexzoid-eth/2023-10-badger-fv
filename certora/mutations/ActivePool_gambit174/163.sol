...
contract ActivePool is IActivePool, ERC3156FlashLender, ReentrancyGuard, BaseMath, AuthNoOwner {
    ...
    function setFeeBps(uint256 _newFee) external requiresAuth {
        /// DeleteExpressionMutation(`ICdpManagerData(cdpManagerAddress).syncGlobalAccountingAndGracePeriod()` |==> `assert(true)`) of: `ICdpManagerData(cdpManagerAddress).syncGlobalAccountingAndGracePeriod(); // Accrue State First`
        assert(true); // Accrue State First
