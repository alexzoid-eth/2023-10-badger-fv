...
contract ActivePool is IActivePool, ERC3156FlashLender, ReentrancyGuard, BaseMath, AuthNoOwner {
    ...
    function sweepToken(address token, uint256 amount) public nonReentrant requiresAuth {
        ...
        /// SwapArgumentsOperatorMutation(`amount <= balance` |==> `balance <= amount`) of: `require(amount <= balance, "ActivePool: Attempt to sweep more than balance");`
        require(balance <= amount, "ActivePool: Attempt to sweep more than balance");
