...
contract ActivePool is IActivePool, ERC3156FlashLender, ReentrancyGuard, BaseMath, AuthNoOwner {
    ...
    function sweepToken(address token, uint256 amount) public nonReentrant requiresAuth {
        ...
        /// DeleteExpressionMutation(`require(amount <= balance, "ActivePool: Attempt to sweep more than balance")` |==> `assert(true)`) of: `require(amount <= balance, "ActivePool: Attempt to sweep more than balance");`
        assert(true);
