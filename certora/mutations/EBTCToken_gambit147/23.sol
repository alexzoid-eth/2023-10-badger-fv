...
contract EBTCToken is IEBTCToken, AuthNoOwner, PermitNonce {
    ...
    function transferFrom(
        ...
        if (cachedAllowance != type(uint256).max) {
            /// SwapArgumentsOperatorMutation(`cachedAllowance >= amount` |==> `amount >= cachedAllowance`) of: `require(cachedAllowance >= amount, "ERC20: transfer amount exceeds allowance");`
            require(amount >= cachedAllowance, "ERC20: transfer amount exceeds allowance");
            unchecked {
