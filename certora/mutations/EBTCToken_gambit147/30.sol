...
contract EBTCToken is IEBTCToken, AuthNoOwner, PermitNonce {
    ...
    function transferFrom(
        ...
        if (cachedAllowance != type(uint256).max) {
            ...
            unchecked {
                /// SwapArgumentsOperatorMutation(`cachedAllowance - amount` |==> `amount - cachedAllowance`) of: `_approve(sender, msg.sender, cachedAllowance - amount);`
                _approve(sender, msg.sender, amount - cachedAllowance);
            }
        }
        return true;
    }
