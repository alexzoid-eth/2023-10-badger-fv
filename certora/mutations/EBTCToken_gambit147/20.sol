...
contract EBTCToken is IEBTCToken, AuthNoOwner, PermitNonce {
    ...
    function transferFrom(
        ...
        if (cachedAllowance != type(uint256).max) {
            /// DeleteExpressionMutation(`require(cachedAllowance >= amount, "ERC20: transfer amount exceeds allowance")` |==> `assert(true)`) of: `require(cachedAllowance >= amount, "ERC20: transfer amount exceeds allowance");`
            assert(true);
            unchecked {
