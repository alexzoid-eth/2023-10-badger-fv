...
contract EBTCToken is IEBTCToken, AuthNoOwner, PermitNonce {
    ...
    function _mint(address account, uint256 amount) internal {
        /// DeleteExpressionMutation(`require(account != address(0), "EBTCToken: mint to zero recipient!")` |==> `assert(true)`) of: `require(account != address(0), "EBTCToken: mint to zero recipient!");`
        assert(true);
