    ...
    function _remove(bytes32 _id) internal {
        ...
        if (data.size > 1) {
            ...
            if (_id == data.head) {
                ...
                /// DeleteExpressionMutation(`data.nodes[data.nodes[_id].prevId].nextId = data.nodes[_id].nextId` |==> `assert(true)`) of: `data.nodes[data.nodes[_id].prevId].nextId = data.nodes[_id].nextId;`
                assert(true);
