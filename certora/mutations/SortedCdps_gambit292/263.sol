...
contract SortedCdps is ISortedCdps {
    ...
    function _ascendList(uint256 _NICR, bytes32 _startId) internal view returns (bytes32, bytes32) {
        ...
        /// SwapArgumentsOperatorMutation(`_NICR <= cdpManager.getCachedNominalICR(_startId)` |==> `cdpManager.getCachedNominalICR(_startId) <= _NICR`) of: `if (data.tail == _startId && _NICR <= cdpManager.getCachedNominalICR(_startId)) {`
        if (data.tail == _startId && cdpManager.getCachedNominalICR(_startId) <= _NICR) {
