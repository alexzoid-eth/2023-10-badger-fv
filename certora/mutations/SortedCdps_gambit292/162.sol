...
contract SortedCdps is ISortedCdps {
    ...
    function batchRemove(bytes32[] memory _ids) external override {
        ...
        for (uint256 i = 0; i < _len; ++i) {
            /// RequireMutation(`contains(_ids[i])` |==> `false`) of: `require(contains(_ids[i]), "SortedCdps: List does not contain the id");`
            require(false, "SortedCdps: List does not contain the id");
        }
