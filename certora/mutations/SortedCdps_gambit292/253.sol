...
contract SortedCdps is ISortedCdps {
    ...
    function _validInsertPosition(
        ...
        if (_prevId == dummyId && _nextId == dummyId) {
            ...
            /// SwapArgumentsOperatorMutation(`_NICR >= cdpManager.getCachedNominalICR(_nextId)` |==> `cdpManager.getCachedNominalICR(_nextId) >= _NICR`) of: `return data.head == _nextId && _NICR >= cdpManager.getCachedNominalICR(_nextId);`
            return data.head == _nextId && cdpManager.getCachedNominalICR(_nextId) >= _NICR;
