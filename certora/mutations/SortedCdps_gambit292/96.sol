...
contract SortedCdps is ISortedCdps {
    ...
    function _insert(bytes32 _id, uint256 _NICR, bytes32 _prevId, bytes32 _nextId) internal {
        ...
        /// RequireMutation(`!contains(_id)` |==> `false`) of: `require(!contains(_id), "SortedCdps: List already contains the node");`
        require(false, "SortedCdps: List already contains the node");
