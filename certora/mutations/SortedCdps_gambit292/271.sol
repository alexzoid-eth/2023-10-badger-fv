...
contract SortedCdps is ISortedCdps {
    ...
    function _findInsertPosition(
        ...
        if (prevId != dummyId) {
            /// IfStatementMutation(`!contains(prevId) || _NICR > cdpManager.getCachedNominalICR(prevId)` |==> `false`) of: `if (!contains(prevId) || _NICR > cdpManager.getCachedNominalICR(prevId)) {`
            if (false) {
