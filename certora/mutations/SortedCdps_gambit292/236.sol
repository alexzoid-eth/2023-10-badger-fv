...
contract SortedCdps is ISortedCdps {
    ...
    function reInsert(
        ...
        require(contains(_id), "SortedCdps: List does not contain the id");
        // NICR must be non-zero
        /// SwapArgumentsOperatorMutation(`_newNICR > 0` |==> `0 > _newNICR`) of: `require(_newNICR > 0, "SortedCdps: NICR must be positive");`
        require(0 > _newNICR, "SortedCdps: NICR must be positive");
