...
contract SortedCdps is ISortedCdps {
    ...
    function _descendList(uint256 _NICR, bytes32 _startId) internal view returns (bytes32, bytes32) {
        ...
        /// IfStatementMutation(`data.head == _startId && _NICR >= cdpManager.getCachedNominalICR(_startId)` |==> `false`) of: `if (data.head == _startId && _NICR >= cdpManager.getCachedNominalICR(_startId)) {`
        if (false) {
