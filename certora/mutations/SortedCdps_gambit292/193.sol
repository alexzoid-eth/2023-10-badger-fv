...
contract SortedCdps is ISortedCdps {
    ...
    function _remove(bytes32 _id) internal {
        ...
        /// DeleteExpressionMutation(`require(contains(_id), "SortedCdps: List does not contain the id")` |==> `assert(true)`) of: `require(contains(_id), "SortedCdps: List does not contain the id");`
        assert(true);
