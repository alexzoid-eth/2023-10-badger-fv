...
contract SortedCdps is ISortedCdps {
    ...
    function _validInsertPosition(
        ...
        if (_prevId == dummyId && _nextId == dummyId) {
            ...
            /// SwapArgumentsOperatorMutation(`_NICR <= cdpManager.getCachedNominalICR(_prevId)` |==> `cdpManager.getCachedNominalICR(_prevId) <= _NICR`) of: `return data.tail == _prevId && _NICR <= cdpManager.getCachedNominalICR(_prevId);`
            return data.tail == _prevId && cdpManager.getCachedNominalICR(_prevId) <= _NICR;
        } else {
