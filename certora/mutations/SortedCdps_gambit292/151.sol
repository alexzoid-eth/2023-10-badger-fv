...
contract SortedCdps is ISortedCdps {
    ...
    function batchRemove(bytes32[] memory _ids) external override {
        ...
        /// SwapArgumentsOperatorMutation(`_len > 1` |==> `1 > _len`) of: `require(_len > 1, "SortedCdps: batchRemove() only apply to multiple cdpIds!");`
        require(1 > _len, "SortedCdps: batchRemove() only apply to multiple cdpIds!");
