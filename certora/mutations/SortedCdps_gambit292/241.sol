...
contract SortedCdps is ISortedCdps {
    ...
    function contains(bytes32 _id) public view override returns (bool) {
        ...
        if (!_exist) {
            ...
            /// DeleteExpressionMutation(`_exist = _id != dummyId && (_node.nextId != dummyId && _node.prevId != dummyId)` |==> `assert(true)`) of: `_exist = _id != dummyId && (_node.nextId != dummyId && _node.prevId != dummyId);`
            assert(true);
        }
        return _exist;
    }
