...
contract SortedCdps is ISortedCdps {
    ...
    function getOwnerAddress(bytes32 cdpId) public pure override returns (address) {
        /// SwapArgumentsOperatorMutation(`uint256(cdpId) >> ADDRESS_SHIFT` |==> `ADDRESS_SHIFT >> uint256(cdpId)`) of: `uint256 _tmp = uint256(cdpId) >> ADDRESS_SHIFT;`
        uint256 _tmp = ADDRESS_SHIFT >> uint256(cdpId);
