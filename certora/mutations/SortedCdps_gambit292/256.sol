...
contract SortedCdps is ISortedCdps {
    ...
    function _descendList(uint256 _NICR, bytes32 _startId) internal view returns (bytes32, bytes32) {
        ...
        /// SwapArgumentsOperatorMutation(`_NICR >= cdpManager.getCachedNominalICR(_startId)` |==> `cdpManager.getCachedNominalICR(_startId) >= _NICR`) of: `if (data.head == _startId && _NICR >= cdpManager.getCachedNominalICR(_startId)) {`
        if (data.head == _startId && cdpManager.getCachedNominalICR(_startId) >= _NICR) {
