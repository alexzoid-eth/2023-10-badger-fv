...
contract SortedCdps is ISortedCdps {
    ...
    function _findInsertPosition(
        ...
        if (nextId != dummyId) {
            /// SwapArgumentsOperatorMutation(`_NICR < cdpManager.getCachedNominalICR(nextId)` |==> `cdpManager.getCachedNominalICR(nextId) < _NICR`) of: `if (!contains(nextId) || _NICR < cdpManager.getCachedNominalICR(nextId)) {`
            if (!contains(nextId) || cdpManager.getCachedNominalICR(nextId) < _NICR) {
